<?php
	...
	*/
	function getConn() {
		$conexao = new PDO('mysql:dbname=atendimentobit;host=atendimentobit.mysql.dbaas.com.br',
		'atendimentobit',
		'l_006023leL40@',
	...
	*/
	$app->group('/chamado',function() use ($app){
		...
		$app->get('/get', function(){
			$stat = getConn()->query("select * from chamados order by idchamado desc");
			$array = array();
	...
	*/
	$app->group('/cliente',function() use ($app){
		...
		$app->get('/get', function(){
			$stat = getConn()->query("select * from clientes order by nome");
			$array = array();
			$array = $stat->fetchAll(PDO::FETCH_OBJ);
			//encerramento da conexão
			$conexao = null;
